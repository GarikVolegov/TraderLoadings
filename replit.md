...
- **Routine** (`/routine`): Morning and evening trader programs with 8 structured steps each. Step-by-step completion tracking (localStorage, resets daily). Morning panel (amber/gold): sveglia mentale → briefing macro → calendario → analisi tecnica → obiettivi → checklist → preparazione fisica → avvia sessione. Evening panel (indigo/purple): review trade → journaling → statistiche → autopsia errori → piano domani → studio → decompressione → disconnetti. Animated progress rings + progress bars, confetti on full completion, "Azzera progressi" reset, links to relevant pages (Zen, News, Tools, Journal, Checklist). RoutineWidget on Dashboard shows compact dual progress bars. Accessible from BottomNav sidebar secondary nav (Sunrise icon).
