...
- **Macro News Ticker (TopNav)**: AI-powered macro news scrolling ticker in the top navigation bar. Fetches OpenAI-generated macro forex briefing via `GET /api/tools/macro-news?currencies=EUR,USD,...`. Shows impact dots (🔴🟡🟢) + currency flags + headline in marquee. Click ticker or Brain icon to open bottom sheet with full article list (title, summary, impact badge, direction), sentiment indicator, and currency filter (multi-select with localStorage persistence). 30-min server-side cache + 30-min client refetch interval. Unified `normalizeCurrencies()` helper ensures consistent cache keys between GET and POST routes.
